...
public class ApiSecurity extends UsernamePasswordAuthenticationFilter {
    ...
    protected void successfulAuthentication(HttpServletRequest request, HttpServletResponse response, FilterChain chain, Authentication authResult) throws IOException, ServletException {
        ...
        Algorithm algorithm = Algorithm.HMAC512( "ismayil".getBytes());

        List<String> list = new ArrayList<>();

        if( userDetails.getAuthorities().stream().anyMatch( a -> a.getAuthority().equals("SITE_USER") )){
            list.add("SITE_USER");
        }

        if( userDetails.getAuthorities().stream().anyMatch( a -> a.getAuthority().equals("ADMIN_USER") )){
            list.add("ADMIN_USER");
        }
